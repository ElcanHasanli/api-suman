...
export async function notifyCourierOnAssign({
  ...
  const orderResult = await pool.query(
    `SELECT o.id, o.address, o.bidons_count, o.order_type,
  ...
  const typeLabel = isPickup ? 'Boş bidon götürmə' : 'Sifariş';

  const message = prevId
    ? `${typeLabel} #${orderId} təyin edildi — ${customerLabel || 'Müştəri'}`
    : `Yeni ${typeLabel.toLowerCase()} #${orderId} — ${customerLabel || 'Müştəri'}`;
  ...
  await sendPushToUser(newId, {
    title: isPickup ? 'Boş bidon götürmə' : 'Yeni sifariş',
    body: message,
    data: {
      type: 'order_assigned',
