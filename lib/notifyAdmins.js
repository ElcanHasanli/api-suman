...
export async function createInAppAdminNotification(
  companyId,
  { message, type, orderId = null, customerId = null }
) {
  ...
  await Promise.all(
    adminIds.map((adminId) =>
      pool.query(
        `INSERT INTO notifications (user_id, order_id, customer_id, type, message)
         VALUES ($1, $2, $3, $4, $5)`,
        [adminId, orderId, customerId, type, message]
      )
    )
  );
...
export async function notifyAdminsWarehouseUpdated(companyId, update, courierUserId) {
  const courierName = await getUserName(courierUserId);
  const warehouseName = update.warehouse_name || update.warehouse_code || 'Anbar';
  const entryFull = Number(update.entry_full ?? update.full_in ?? 0);
  const entryEmpty = Number(update.entry_empty ?? update.empty_in ?? 0);
  const exitFull = Number(update.exit_full ?? 0);
  const fullTaken = Number(update.full_taken ?? update.full_out ?? 0);
  const remFull = Number(update.remaining_full ?? 0);
  const remEmpty = Number(update.remaining_empty ?? 0);

  const body = `${courierName} (${warehouseName}): girdi ${entryFull} dolu + ${entryEmpty} boş, çıxdı ${exitFull} dolu (götürdü ${fullTaken}) → anbarda ${remFull} dolu, ${remEmpty} boş`;
  ...
  return notifyCompanyAdmins(companyId, {
    ...
    data: {
      ...
      warehouse_id: update.warehouse_id ? String(update.warehouse_id) : '',
      screen: 'warehouse',
    },
  });
}
...
export async function notifyAdminsCustomerInactive(companyId, customer) {
  ...
  const bidons = Number(customer.active_bidons ?? 0);
  const message = `${label} 20 gündür sifariş verməyib — qalıq ${bidons} bidon (son: ${lastOrderDate})`;
  ...
  return notifyCompanyAdmins(companyId, {
    title: 'Passiv müştəri (qalıq bidon)',
    body: message,
    data: {
      type: 'customer_inactive',
      customer_id: String(customer.id),
      screen: 'customers',
      last_order_date: String(lastOrderDate),
      active_bidons: String(bidons),
      inactivity_days: '20',
    },
  });
}
